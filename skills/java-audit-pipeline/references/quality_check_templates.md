...
- [阶段4 校验清单](#阶段4-校验清单)
  ...
  - [agent-8 exploit-chain 校验清单](#agent-8-exploit-chain-校验清单)
...
### agent-8-exploit-chain 校验清单

校验对象: `{output_path}/cross_analysis/exploit_chains.md`

| # | 校验项 | 预期 | 实际 | 状态 |
|---|--------|------|------|------|
| 1 | 利用链报告存在且非空 | `exploit_chains.md` 存在，长度 > 0 | {填写} | {✅/❌} |
| 2 | 已读取所有 agent-6x 输出 | 报告中引用的漏洞均来自 agent-6x 报告 | {填写} | {✅/❌} |
| 3 | 8 种模板均已匹配 | 8 种利用链模板均评估过（含"不适用"标记） | {填写} | {✅/❌} |
| 4 | 每个利用链含前置条件评估 | 每个链都有 [x]/[ ] checklist | {填写} | {✅/❌} |
| 5 | 利用链按可达性排序 | P0 可直接利用的链排在前面 | {填写} | {✅/❌} |
| 6 | 独立 RCE 漏洞优先列出 | 表达式注入等独立 RCE 在利用链报告中优先展示 | {填写} | {✅/❌} |
| 7 | 无遗漏可组合漏洞 | agent-6x 中所有未达 Critical 但有组合潜力的漏洞均已评估 | {填写} | {✅/❌} |
````
